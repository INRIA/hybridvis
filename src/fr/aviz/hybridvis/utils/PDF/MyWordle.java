...
/*
 * Adapted from MyWordle.java by lindenb 
...
public class MyWordle {
	...
	private double dRadius = 20.0;
	private int dDeg = 10;
	private boolean useArea = false;
	private int doSortType = 2;
	...
	public void setFontFaminly(String ff) {
		this.fontFamily = ff;
	}

	public void setBigSize(int bs) {
		this.biggestSize = bs;
	}

	public void setSmallSize(int ss) {
	...
	public void doLayout() {
		...
		for (int i = 1; i < this.words.size(); ++i) {
			...
			boolean done = false;
			double radius = 0.5 * Math.min(first.bounds.getWidth(),
					first.bounds.getHeight());

			while (!done) {
				System.err.println("" + i + "/" + words.size() + " rad:"
						+ radius);
				...
				for (int deg = startDeg; deg < startDeg + 360; deg += dDeg) {
					...
					AffineTransform moveTo = AffineTransform
							.getTranslateInstance(cx, cy);
					Shape candidate = moveTo
							.createTransformedShape(current.shape);
					Area area1 = null;
					Rectangle2D bound1 = null;
					if (useArea) {
						area1 = new Area(candidate);
					} else {
						bound1 = new Rectangle2D.Double(current.bounds.getX()
								+ cx, current.bounds.getY() + cy,
		...
		AffineTransform shiftTr = AffineTransform.getTranslateInstance(-minx,
				-miny);
		for (Word w : words) {
	...
	private void random() {
		for (int i = 0; i < 500; ++i) {
			...
			Color c = new Color(rand.nextInt(100), rand.nextInt(100),
					rand.nextInt(100));
			w.setFill(c);
			c = new Color(rand.nextInt(100), rand.nextInt(100),
					rand.nextInt(100));
			w.setStroke(c);
			w.setTitle("" + i);
	...
	public BufferedImage returnImg() {
		...
		Dimension dim = new Dimension((int) this.imageSize.getWidth(),
		...
		for (Word w : this.words) {
			...
			if (c != null) {
				...
				g.setStroke(new BasicStroke(w.getLineHeight(),
	...
	public void saveAsPostscript(File file) throws IOException {
		...
		out.println("%%%BoundingBox: 0 0 " + (int) this.imageSize.getWidth()
				+ " " + (int) this.imageSize.getHeight());
	...
	public static void main(String[] args) {
		try {
			...
			if (fileOut.getName().toLowerCase().endsWith(".svg")
					|| (format != null && format.equalsIgnoreCase("svg"))) {
				app.saveAsSVG(fileOut);
			} else if (fileOut.getName().toLowerCase().endsWith(".png")
					|| (format != null && format.equalsIgnoreCase("png"))) {
				app.saveAsPNG(fileOut);
			} else if (fileOut.getName().toLowerCase().endsWith("ps")
					|| (format != null && format.equalsIgnoreCase("ps"))) {
