...
public class GUIUtils {
	...
	public static void drawText(Graphics2D g, String s, double x, double y, double maxwidth, double maxheight, HALIGN halign, VALIGN valign, double angle, Color bgColor, Color borderColor, int borderMargin, boolean mirrored) {
		...
		double scaley = 1;
		if (bwidth > maxwidth)
			scalex = maxwidth / bwidth;
		if (bheight > maxheight)
			scaley = maxheight / bheight;
...
//	    g.setColor(c0);
	...
	public static void drawCheckerBoard(Graphics2D g, Rectangle2D bounds, int size, Color c1, Color c2) {
		// -- Checker board (for transparent images)
		g.setColor(c1);
		g.fill(bounds);
		g.setColor(c2);
		boolean even = true;
		int bx0, bx1, by0, by1;
		bx0 = (int)bounds.getMinX();
		bx1 = (int)bounds.getMaxX();
		by0 = (int)bounds.getMinY();
		by1 = (int)bounds.getMaxY();
		int x0, y0, x1, y1;
		for (y0 = by0; y0 < by1; y0 += size) {
			y1 = Math.min(y0 + size, by1);
			for (x0 = bx0 + (even ? size : 0); x0 < bx1; x0 += 2 * size) {
				x1 = Math.min(x0 + size, bx1);
